...
namespace Blockera\SiteBuilder;
...
final class StyleEngine {
	...
	protected function setDefinition( array $supports, string $id): void {
		...
		try {
			...
			$abstract = '\Blockera\SiteBuilder\StyleDefinitions\\' . $supports[ $id ]['definition'];
							...
							return blockera_array_flat(
								array_map(
									function ( array $settings, string $state) use ( $flattenSupports, $blockType): array {
										...
										return blockera_array_flat(
											array_map(
												function ( array $breakpointSettings) use ( $flattenSupports, $blockType, $state, $settings): array {
													return blockera_array_flat(
														array_map(
															function ( $_settings, string $id) use ( $flattenSupports, $blockType, $state, $settings): array {
